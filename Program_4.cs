...
{
    /// <summary>
    /// 문제 4: 다중 에이전트 AI 모델 정확도 집계 서비스
    /// 여러 AI 모델의 AgentId를 MODELS.JSON에서 읽고, 각 모델별 정확도를 독립적으로 계산합니다.
    /// 
    /// 주요 기능:
    /// - MODELS.JSON 파일 로드: 다중 모델명별 AgentId 매핑 정보 읽기
    /// - POST /monitoring: 모니터링 데이터(JSON) 수신 및 In-Memory 저장
    /// - GET /accuracy?modelName=&timeWindow=: 특정 AI 모델의 정확도 조회
    /// </summary>
    class Program
    {
        // ==================== 전역 데이터 저장소 ====================
        ...
        /// 각 항목은 RequestId, Timestamp, AgentId, DataType(P/A), DataValue를 포함합니다.
        ...
        /// 예: {"modelA": ["agent1", "agent2"], "modelB": ["agent3", "agent4"]}
        /// </summary>
        ...
        /// </summary>
        /// <param name="args">명령줄 인수 (현재는 사용되지 않음)</param>
        static void Main(string[] args)
        {
            ...
            // 비동기로 HTTP 요청 수신 (프로그램이 종료되지 않도록 백그라운드 태스크에서 실행)
            Task.Run(() => ListenForRequests(listener));
            
            // 프로그램이 계속 실행되도록 콘솔 입력 대기
            // 이 줄 때문에 프로그램이 즉시 종료되지 않고 요청 수신을 기다립니다.
            Console.ReadLine();
        }
        ...
        /// 여러 모델이 지원되며, 각 모델은 복수의 AgentId를 가질 수 있습니다.
        ...
        static void LoadModelsInfo()
        {
            ...
            if (File.Exists(modelsFile))
            {
                ...
                try
                {
                    ...
                    {
                        string modelName = prop.Name;    // 모델명 (예: "modelA", "modelB")
                ...
                {
                    // JSON 파싱 실패 시 빈 값 사용 (에러 무시하고 빈 상태로 진행)
                }
            }
        }
        ...
        /// 클라이언트가 연결될 때까지 블로킹으로 대기하다가 도착한 요청을 
        /// 백그라운드 태스크로 전달하여 동시 다중 요청을 처리합니다.
        /// </summary>
        ...
        /// <summary>
        /// 단일 HTTP 요청을 처리합니다.
        /// 요청의 HTTP 메서드와 경로에 따라 적절한 처리를 수행합니다.
        /// 
        /// 지원 엔드포인트:
        /// - POST /monitoring: 모니터링 데이터(JSON) 수신
        /// - GET /accuracy?modelName=&timeWindow=: AI 모델 정확도 조회
        /// </summary>
        static void HandleRequest(HttpListenerContext context)
        {
            ...
            try
            {
                ...
                if (request.HttpMethod == "POST" && request.Url?.AbsolutePath == "/monitoring")
                {
                    ...
                    using (var reader = new StreamReader(request.InputStream))
                    {
                        ...
                        // 수신된 모니터링 데이터를 처리하여 In-Memory 저장소에 추가합니다.
                ...
                else if (request.HttpMethod == "GET" && request.Url?.AbsolutePath == "/accuracy")
                {
                    ...
                    // 필수 파라미터가 누락된 경우 400 Bad Request 에러 반환
                ...
                // 지원하지 않는 경로 또는 메서드인 경우 404 Not Found 반환
                else
                {
            ...
            {
                // 응답 스트림 닫기 (정상/비정상 종료 관계없이 항상 실행)
                response.OutputStream.Close();
            }
        }

        /// <summary>
        /// 수신된 모니터링 JSON 데이터를 파싱하여 In-Memory 저장소에 추가합니다.
        /// 단일 객체 또는 배열 형식 모두 지원합니다.
        /// 
        /// JSON 형식:
        ///   단일: {"requestId": "...", "timestamp": "...", "agentId": "...", "dataType": "...", "dataValue": "..."}
        ///   배열: [{...}, {...}, ...]
        /// </summary>
        static void ProcessMonitoringData(string json)
        {
            // 스레드 안전성을 위해 lock으로 보호 (동시 접근 방지)
            lock (lockObj)
            {
                try
                {
        ...
        /// 
        /// 정확도 계산 로직:
        /// 1. MODELS.JSON에서 modelName에 해당하는 AgentId 목록 조회
        /// 2. timeWindow(yyyyMMddHH 형식)에 해당하는 시간 범위 설정
        /// 3. 해당 에이전트들의 P(예측 값) 데이터 중 시간 범위 내의 것만 필터링
        /// 4. 동일한 requestId를 가진 P와 A(실제 값) 쌍 찾기
        /// 5. P == A인 쌍의 수 / 총 쌍의 수 로 정확도 계산
        /// </summary>
        /// <param name="modelName">조회할 모델명 (MODELS.JSON에 등록된 이름)</param>
        /// <param name="timeWindow">시간 범위 (yyyyMMddHH 형식, 예: "2025041514" = 2025년 4월 15일 14시)</param>
        ...
        static string CalculateAccuracy(string modelName, string timeWindow)
        {
            ...
            lock (lockObj)
            {
                // 모델명이 modelsInfo에 없는 경우: 0/0 반환 (데이터 없음)
        ...
        /// URL 디코딩도 함께 수행하여 특수문자가 포함된 값도 올바르게 가져옵니다.
        /// </summary>
    ...
    /// <summary>
    /// 모니터링 데이터를 나타내는 데이터 구조체 (DTO: Data Transfer Object)
    /// HTTP API를 통해 수신된 데이터를 저장하는 데 사용됩니다.
    /// </summary>
    class MonitoringData
    {
        /// <summary>요청 고유 ID - 동일한 P/A 쌍을 연결하는 키</summary>
        ...
        /// <summary>데이터 값 - P는 AI의 예측 결과, A는 정답</summary>
